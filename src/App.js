...
import { FaFolder, FaChevronLeft, FaChevronRight, FaSearch, FaDownload, FaExclamationTriangle, FaArrowsAlt } from "react-icons/fa";
...
function App() {
    ...
    const filteredMenuItems = [
        ...
        { id: "move", label: "Move ➡️", icon: <FaArrowsAlt /> },
    ...
    return (
        <div className="d-flex vh-100 flex-column">
            ...
            <div className="d-flex flex-grow-1">
                <aside className={`bg-secondary text-white d-flex flex-column position-relative`} style={{ width: isSidebarOpen ? "250px" : "80px", transition: "width 0.3s", padding: isSidebarOpen ? "1rem" : "0.5rem", backgroundColor: darkBlue }}>
                    <h1 className={`h6 mb-4 ${isSidebarOpen ? "" : "d-none"}`} style={{ color: "white" }}>Menu</h1>
                    <div className="mb-3">
                        <div className="input-group">
                            <input type="text" className={`form-control ${isSidebarOpen ? "" : "d-none"}`} placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} aria-label="Search menu items" />
                            <button className="btn btn-light"><FaSearch /></button>
                        </div>
                    </div>

                    <button className="btn btn-light w-100 mb-3 d-flex align-items-center justify-content-between" onClick={toggleMenu} aria-expanded={isOpen}>
                    ...
                    <button className="btn btn-outline-light position-absolute" onClick={toggleSidebar} style={{ bottom: "1rem", right: "1rem" }}>
                ...
                <main className="flex-grow-1 p-4">
                        ...
                        <form className="border p-3 rounded" style={{ width: "400px" }}>
                            ...
                            <div className="d-flex justify-content-between">
                                <div className="mb-3" style={{ width: "48%" }}>
                                    ...
                                    <input type="date" className="form-control form-control-sm" id="startDate" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
                                </div>
                                <div className="mb-3" style={{ width: "48%" }}>
                                    <label htmlFor="endDate" className="form-label" style={{ fontSize: "14px" }}>Fin</label>
                                    <input type="date" className="form-control form-control-sm" id="endDate" value={endDate} onChange={(e) => setEndDate(e.target.value)} />
                                </div>
                            </div>
                            <div className="mb-3">
                                <label htmlFor="partenaire" className="form-label" style={{ fontSize: "14px" }}>Partenaire</label>
                                <input type="number" className="form-control form-control-sm" id="partenaire" value={partenaire} onChange={(e) => setPartenaire(e.target.value)} />
                            </div>
                        </form>
                    </div>

                    {/* Affichage du contenu du tab actif uniquement */}
                    {activeTab === "download" && (
                    ...
                    {activeTab === "verification-error" && (
                        <div className="text-center py-5">
                            <p>Erreur de vérification : Aucune donnée disponible</p>
                        </div>
                    )}

                    {activeTab === "move" && (
                        <div className="text-center py-5">
                            <p>Déplacer : Aucune donnée disponible</p>
                        </div>
                    )}
